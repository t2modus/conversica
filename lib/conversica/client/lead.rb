  ...
  module Client
    ...
    class Lead
      ...
      PERMITTED_COLUMNS = %w[
        ...
        cell_phone city client_id conversation_id date email first_name home_email home_phone id last_name lead_source
        lead_status lead_type make model opt_out rep_id rep_email rep_name service_rep_id service_rep_email
        service_rep_name sms_opt_out state skip_to_followup stop_messaging vin work_email work_phone year zip
      ].freeze

      BOOLEAN_COLUMNS = Set.new(
        %w[stop_messaging skip_to_followup opt_out sms_opt_out]
      ).freeze

      DATE_COLUMNS = Set.new(
        %w[date appointment_date]
      ).freeze

      REQUIRED_ATTRIBUTES = %w[
        api_version bdc_rep_email bdc_rep_name conversation_id email first_name id lead_source lead_status make model
        rep_email rep_name year
      ].freeze
      ...
      def validate_type
        PERMITTED_COLUMNS.each do |k|
          permitted_classes, error = if BOOLEAN_COLUMNS.include?(k)
                                       ...
                                       [
                                         [NilClass, Date, Time],
                                         "#{k} must be null or a valid date or time object"
                                       ]
                                     else
                                       [
                                         [NilClass, String],
                                         "#{k} must be null or a string"
                                       ]
                                     end
          ...
          # Class === instance_of_class is true, but instance_of_class === Class
          # is not
      ...
      def conversicate
        ...
        DATE_COLUMNS.each_with_object(hash) do |k, h|
          value = h[k]
          next if value.nil?
          h[k] = value.to_datetime.rfc3339
        end
